...
import { ArrowLeft, BarChart3, Bot, CheckCircle2, ChevronRight, ClipboardList, Megaphone, PhoneCall, Plus, ShieldCheck, Target, Trash2, Users } from "lucide-react";
...
type CampaignIteration = { id: string; campaign_id?: string | null; study_id: string; iteration_number: number; iteration_name: string; research_phase: string; status: string; target_sample_size: number | null; planned_start_date: string | null; run_count?: number; completed_run_count?: number; allocation_editable?: boolean; successful_voters?: number; voice_agent_id?: string | null; voice_agent_name?: string | null; voice_agent_category?: string | null; voice_agent_app_id?: string | null; voice_agent_app_version?: number | null; questionnaire_id?: string | null; questionnaire_code?: string | null; questionnaire_name?: string | null; questionnaire_version?: number | null };
...
export default function CampaignDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [questionnaires, setQuestionnaires] = useState<IterationQuestionnaire[]>([]);
  ...
  const [iterationForm, setIterationForm] = useState({ name: "", stage: "BASE", targetSample: "", voiceAgentId: "", questionnaireId: "", startDate: "", endDate: "" });
  const [editingAgentIterationId, setEditingAgentIterationId] = useState<string | null>(null);
  const [selectedAgentVersionId, setSelectedAgentVersionId] = useState("");
  const [savingAgentVersion, setSavingAgentVersion] = useState(false);
      ...
      try {
        ...
        if (user?.role.code !== "CAMPAIGNER") {
          ...
          if (user?.role.code === "CAMPAIGN_MANAGER") {
            ...
            try {
              const questionnaireResult = await apiFetch(`/api/campaigns/${id}/iteration-questionnaires`);
              if (!cancelled) setQuestionnaires(Array.isArray(questionnaireResult) ? questionnaireResult : questionnaireResult.items || []);
            } catch (reason) {
              if (!cancelled) setIterationError(reason instanceof Error ? reason.message : "Iteration questionnaires are not available yet");
            }
          }
  ...
  async function completeCampaign() {
    ...
    try {
      ...
      setLifecycle(function (current) {
        return {
          ...result,
          blockers: Array.isArray(result.blockers) ? result.blockers : [],
          history: Array.isArray(result.history) ? result.history : current?.history || []
        };
      });
  ...
  async function saveIterationAllocations() {
    ...
    try {
      ...
      const confirmation = `${saved.length} work area${saved.length === 1 ? "" : "s"} saved. Campaigners can now access only the geography allocated to them.`;
      setAllocationNotice(confirmation);
  ...
  async function createCampaignIteration() {
    if (!campaign || !iterationForm.name.trim() || !iterationForm.targetSample || Number(iterationForm.targetSample) < 1 || !iterationForm.voiceAgentId || !iterationForm.questionnaireId) {
      setIterationError("Iteration name, target sample, questionnaire and Sarvam voice agent are required.");
      return;
    }
    ...
    try {
      const result = await apiFetch(`/api/campaigns/${id}/iterations`, {
        ...
        body: JSON.stringify({
          ...
          questionnaireId: iterationForm.questionnaireId,
      ...
      setIterationForm(function (current) { return { ...current, name: "", stage: "BASE", voiceAgentId: "", questionnaireId: "", startDate: "", endDate: "" }; });
  ...
  function openAgentVersionEditor(iteration: CampaignIteration) {
    const next = voiceAgents.find(function (agent) {
      return agent.app_id === iteration.voice_agent_app_id &&
        Number(agent.app_version) > Number(iteration.voice_agent_app_version || 0);
    });
    setEditingAgentIterationId(iteration.id);
    setSelectedAgentVersionId(next?.id || "");
    setIterationError(null);
  }

  async function saveAgentVersion() {
    if (!editingAgentIterationId || !selectedAgentVersionId) {
      setIterationError("Select a newer committed version of this Sarvam Agent App.");
      return;
    }
    const current = iterations.find(function (iteration) { return iteration.id === editingAgentIterationId; });
    const selected = voiceAgents.find(function (agent) { return agent.id === selectedAgentVersionId; });
    if (!current || !selected) return;
    if (!window.confirm(`Change ${current.iteration_name} from v${current.voice_agent_app_version || "?"} to v${selected.app_version}? Previous call evidence remains on its original version; only future calls use v${selected.app_version}.`)) return;
    setSavingAgentVersion(true);
    setIterationError(null);
    setActionNotice(null);
    try {
      const updated = await apiFetch(`/api/campaigns/${id}/iterations/${editingAgentIterationId}/voice-agent`, {
        method: "PATCH",
        body: JSON.stringify({ voiceAgentId: selectedAgentVersionId })
      });
      setIterations(function (currentRows) {
        return currentRows.map(function (iteration) {
          return iteration.id === editingAgentIterationId ? { ...iteration, ...updated } : iteration;
        });
      });
      setActionNotice(`${current.iteration_name} now uses ${updated.voice_agent_name || updated.voice_agent_app_id} v${updated.voice_agent_app_version} for future calls. ${Number(updated.prior_executions_preserved || 0)} prior execution${Number(updated.prior_executions_preserved || 0) === 1 ? " was" : "s were"} preserved with original version evidence.`);
      setEditingAgentIterationId(null);
      setSelectedAgentVersionId("");
    } catch (reason) {
      setIterationError(reason instanceof Error ? reason.message : "Unable to change the Iteration agent version");
    } finally {
      setSavingAgentVersion(false);
    }
  }

  const canReview = Boolean(user && ["SUPER_ADMIN", "ADMIN"].includes(user.role.code));
  const canViewAnalysis = Boolean(user && ["SUPER_ADMIN", "ADMIN", "CAMPAIGN_MANAGER"].includes(user.role.code));
  ...
  const selectedIteration = iterations.find(function (iteration) { return iteration.id === selectedIterationId; });
  const selectedIterationClosed = ["COMPLETED", "LOCKED"].includes(String(selectedIteration?.status || "").toUpperCase());
  const canAllocateIteration = canManageIterations && Boolean(selectedIterationId) && !selectedIterationClosed;
  const agentVersionIteration = iterations.find(function (iteration) { return iteration.id === editingAgentIterationId; });
  const agentVersionOptions = voiceAgents.filter(function (agent) {
    return agent.app_id === agentVersionIteration?.voice_agent_app_id &&
      Number(agent.app_version) > Number(agentVersionIteration?.voice_agent_app_version || 0);
  });
  ...
  const iterationProgress = useMemo(function () {
    const completed = iterations.filter(function (iteration) {
      return ["COMPLETED", "LOCKED"].includes(String(iteration.status).toUpperCase()) || Number(iteration.completed_run_count || 0) >= 3;
    }).length;
    const inProcess = iterations.filter(function (iteration) {
      const status = String(iteration.status).toUpperCase();
      return !["COMPLETED", "LOCKED"].includes(status) && Number(iteration.completed_run_count || 0) < 3 && (
        Number(iteration.run_count || 0) > 0 ||
    ...
    return {
      ...
      completed,
      inProcess,
      notStarted: Math.max(iterations.length - completed - inProcess, 0)
    };
  }, [iterations]);
  const allocationStateByIteration = useMemo(function () {
    const states = new globalThis.Map<string, {
    ...
    for (const iteration of iterations) {
      const allocations = (campaign?.allocations || []).filter(function (allocation) {
        return allocation.iteration_id === iteration.id;
      });
      const campaignerNames = Array.from(new Set(allocations.map(function (allocation) {
        return allocation.campaigner_name;
      }).filter(Boolean))).sort();

      if (localAreas.length) {
        const assignedAreaIds = new Set(allocations.map(function (allocation) {
          return allocation.local_body_area_id;
        }).filter(Boolean));
        states.set(iteration.id, {
          assignedAreas: assignedAreaIds.size,
          totalAreas: localAreas.length,
          complete: localAreas.length > 0 && assignedAreaIds.size >= localAreas.length,
          campaignerNames
        });
        continue;
      }

      const assignedGeographies = new Set(allocations.map(function (allocation) {
        return allocation.geo_unit_id;
      }).filter(Boolean));
      const assignedAreas = (campaign?.scope || []).filter(function (scope) {
        return assignedGeographies.has(scope.id) || assignedGeographies.has(scope.district_id);
      }).length;
      const totalAreas = campaign?.scope.length || 0;
      states.set(iteration.id, {
        assignedAreas,
        totalAreas,
        complete: totalAreas > 0 && assignedAreas >= totalAreas,
        campaignerNames
      });
    }

    return states;
  }, [campaign, iterations, localAreas]);
  ...
  return <AppShell><div className={styles.page}>
    ...
    {error ? <FeedbackMessage message={error} className={styles.message} /> : !campaign ? <div className={styles.empty}>Loading campaign…</div> : <>
      ...
      <section className={styles.iterationPanel}>
        ...
        {showIterationForm && canManageIterations && <div className={styles.iterationForm}><label>Iteration name<input value={iterationForm.name} onChange={function (event) { setIterationForm({ ...iterationForm, name: event.target.value }); }} placeholder="Base survey — September" /></label><label>Survey stage<select value={iterationForm.stage} onChange={function (event) { setIterationForm({ ...iterationForm, stage: event.target.value }); }}>{surveyStageOptions.map(function (option) { return <option key={option.value} value={option.value}>{option.label}</option>; })}</select></label><label>AI voice agent *<select value={iterationForm.voiceAgentId} onChange={function (event) { setIterationForm({ ...iterationForm, voiceAgentId: event.target.value }); }}><option value="">Select Sarvam Agent App</option>{voiceAgents.map(function (agent) { return <option key={agent.id} value={agent.id}>{formatAgentCategory(agent.usage_category)} · {agent.provider_name || agent.app_id} · v{agent.app_version}</option>; })}</select></label><label>Research questionnaire *<select value={iterationForm.questionnaireId} onChange={function (event) { setIterationForm({ ...iterationForm, questionnaireId: event.target.value }); }}><option value="">Select questionnaire and version</option>{questionnaires.map(function (questionnaire) { return <option key={questionnaire.id} value={questionnaire.id}>{questionnaire.questionnaire_name} · {questionnaire.questionnaire_code} · v{questionnaire.version_number} · {questionnaire.status}</option>; })}</select></label><label>Target sample<input type="number" min={1} value={iterationForm.targetSample} onChange={function (event) { setIterationForm({ ...iterationForm, targetSample: event.target.value }); }} /></label><label>Planned start<input type="date" value={iterationForm.startDate} onChange={function (event) { setIterationForm({ ...iterationForm, startDate: event.target.value }); }} /></label><label>Planned end<input type="date" value={iterationForm.endDate} onChange={function (event) { setIterationForm({ ...iterationForm, endDate: event.target.value }); }} /></label><div className={styles.iterationFormActions}><button type="button" className={styles.secondaryButton} onClick={function () { setShowIterationForm(false); }}>Cancel</button><button type="button" className={styles.primaryAction} disabled={savingIteration || !voiceAgents.length || !questionnaires.length} onClick={createCampaignIteration}>{savingIteration ? "Creating…" : "Create iteration"}</button></div>{!questionnaires.length && <FeedbackMessage message="No questionnaire is available for this campaign. Ask an Admin to create one before starting another iteration." className={styles.message} />}{!voiceAgents.length && <FeedbackMessage message="No campaign-ready Sarvam Agent App is available. Ask an Admin to register and categorize the agents first." className={styles.message} />}</div>}
        {iterations.length ? <div className={styles.iterationList}>{iterations.map(function (iteration) { const stage = surveyStageOptions.find(function (option) { return option.value === String(iteration.research_phase).toUpperCase(); }); const iterationClosed = ["COMPLETED", "LOCKED"].includes(String(iteration.status).toUpperCase()) || Number(iteration.completed_run_count || 0) >= 3; const executionStarted = Number(iteration.run_count || 0) > 0; const allocationEditable = iteration.allocation_editable !== false; const allocationState = allocationStateByIteration.get(iteration.id); const campaignerNames = allocationState?.campaignerNames || []; const lifecycleLabel = iterationClosed ? "Completed" : executionStarted ? "In process" : "Not started"; const newerVersionAvailable = voiceAgents.some(function (agent) { return agent.app_id === iteration.voice_agent_app_id && Number(agent.app_version) > Number(iteration.voice_agent_app_version || 0); }); return <Link key={iteration.id} href={`/iterations/${iteration.id}`} className={styles.iterationRow}><div className={styles.iterationNumber}>{iteration.iteration_number}</div><div><strong>{iteration.iteration_name}</strong><span>{stage?.label || iteration.research_phase} · {lifecycleLabel}</span><small><Bot size={13} />{iteration.voice_agent_name || "Voice agent not assigned"}{iteration.voice_agent_app_version ? ` · v${iteration.voice_agent_app_version}` : ""}{iteration.voice_agent_category ? ` · ${formatAgentCategory(iteration.voice_agent_category as VoiceAgent["usage_category"])}` : ""}</small><small><ClipboardList size={13} />{iteration.questionnaire_id ? `${iteration.questionnaire_name || iteration.questionnaire_code || "Questionnaire"}${iteration.questionnaire_version ? ` · v${iteration.questionnaire_version}` : ""}` : "Questionnaire not identified"}</small>{campaignerNames.length > 0 && <small className={styles.iterationCampaigners}><Users size={13} />{campaignerNames.length === 1 ? "Campaigner" : "Campaigners"}: {campaignerNames.join(", ")}</small>}</div><div><small><Target size={13} />{Number(iteration.target_sample_size || 0).toLocaleString()} voters</small><small><PhoneCall size={13} />{Number(iteration.run_count || 0)} runs</small><small><CheckCircle2 size={13} />{Number(iteration.successful_voters || 0)} successful</small></div><div className={styles.iterationRowActions}>{canManageIterations && !iterationClosed && newerVersionAvailable && <button type="button" className={styles.inlineAction} onClick={function (event) { event.preventDefault(); event.stopPropagation(); openAgentVersionEditor(iteration); }}>Change agent version</button>}{canManageIterations && !iterationClosed && allocationEditable ? <button type="button" className={styles.inlineAction} onClick={function (event) { event.preventDefault(); event.stopPropagation(); openIterationAllocations(iteration.id); }}>{selectedIterationId === iteration.id ? "Editing allocation" : executionStarted ? "Reassign work" : Number(allocationState?.assignedAreas || 0) > 0 ? "Manage allocation" : "Allocate work"}</button> : allocationState?.complete ? <span className={executionStarted && !iterationClosed ? styles.lockedAllocationState : styles.allocatedState}>{iterationClosed ? <><CheckCircle2 size={14} />Completed</> : executionStarted ? "Run in progress · allocation locked" : <><CheckCircle2 size={14} />Allocated</>}</span> : executionStarted ? <span className={styles.lockedAllocationState}>Run in progress · allocation locked</span> : null}</div><ChevronRight size={17} /></Link>; })}</div> : <div className={styles.empty}><ClipboardList size={24} /><strong>No campaign iterations yet</strong><span>{canManageIterations ? "Create the first survey wave and choose its Sarvam voice agent." : "Iterations will appear here once the Campaign Manager creates them."}</span></div>}
        {canManageIterations && agentVersionIteration && <section className={styles.agentVersionPanel}><div><span>VOICE AGENT VERSION</span><h3>{agentVersionIteration.iteration_name}</h3><p>Current: {agentVersionIteration.voice_agent_name || agentVersionIteration.voice_agent_app_id} v{agentVersionIteration.voice_agent_app_version}. Earlier executions keep their original version evidence; only future calls move to the selected version.</p></div><label>New committed version<select value={selectedAgentVersionId} onChange={function (event) { setSelectedAgentVersionId(event.target.value); }}><option value="">Select newer version</option>{agentVersionOptions.map(function (agent) { return <option key={agent.id} value={agent.id}>{agent.provider_name || agent.app_id} · v{agent.app_version}</option>; })}</select></label><div className={styles.agentVersionActions}><button type="button" className={styles.secondaryButton} disabled={savingAgentVersion} onClick={function () { setEditingAgentIterationId(null); setSelectedAgentVersionId(""); }}>Cancel</button><button type="button" className={styles.primaryAction} disabled={savingAgentVersion || !selectedAgentVersionId} onClick={saveAgentVersion}>{savingAgentVersion ? "Updating…" : "Update future calls"}</button></div>{!agentVersionOptions.length && <FeedbackMessage message="No newer selectable version of this Agent App is available. Ask an Admin to synchronize or register the committed version." className={styles.message} />}</section>}
...
function lifecycleEventLabel(event: CampaignLifecycle["history"][number]) {
  if (event.entity_type === "RUN" || event.entity_type === "CALL_EXECUTION") {
    const iteration = event.iteration_number ? `ITERATION ${event.iteration_number}` : "ITERATION";
    const run = event.run_number ? `RUN ${event.run_number}` : event.entity_type.replace("_", " ");
    return `${iteration} · ${run}`;
  }
  if (event.entity_type === "ITERATION" && event.iteration_number) {
    return `ITERATION ${event.iteration_number}`;
  }
