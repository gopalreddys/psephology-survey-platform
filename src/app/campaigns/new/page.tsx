...
export default function NewCampaignPage() {
  ...
  }, [scopeLinks, geographies, geoById]);
  ...
  return <AppShell><div className={styles.page}>
    ...
    <section className={styles.createPanel}>
      ...
      <div className={styles.scopeSelector}><Field label="Select election geography"><select value={targetId} onChange={function (event) { chooseTarget(event.target.value); }}><option value="">Select target</option>{availableTargets.map(function (item) { return <option key={item.id} value={item.id}>{item.name}{item.code ? ` (${item.code})` : ""}</option>; })}</select></Field><div className={styles.scopeSummary}><MapPin size={18} /><div><strong>{target?.name || "No target selected"}</strong><span>{target ? `${districts.length} Districts · ${mandals.length} Mandals resolved` : "Choose a target to load its verified geography."}</span></div></div></div>
    </section>
    {target && <section className={styles.createPanel}>
      <div className={styles.panelHeader}><div><span>STEP 3</span><h2>Allocate campaign work</h2><p>{urbanAreaMode ? "Assign divisions or wards directly." : "Assign a complete District or individual Mandals. A Mandal cannot overlap an assigned District."}</p></div><strong>{allocationCount} allocations</strong></div>
      {urbanAreaMode ? <div className={styles.areaAllocation}>{localAreas.map(function (area) { return <label key={area.id}><span><strong>{area.display_label || area.name}</strong><small>{area.code || area.area_type}</small></span><CampaignerSelect value={assignments[`A:${area.id}`] || ""} campaigners={campaigners} onChange={function (value) { setAssignments({ ...assignments, [`A:${area.id}`]: value }); }} /></label>; })}</div> : <div className={styles.allocationBody}><label className={styles.search}><Search size={15} /><input value={search} onChange={function (event) { setSearch(event.target.value); }} placeholder="Search District or Mandal" /></label><div className={styles.districtGroups}>{visibleDistricts.map(function (group) { const districtValue = assignments[`D:${group.district.id}`] || ""; return <article key={group.district.id}><div className={styles.districtHead}><span><strong>{group.district.name}</strong><small>{group.mandals.length} Mandals in campaign scope</small></span><CampaignerSelect value={districtValue} campaigners={campaigners} onChange={function (value) { assignDistrict(group.district.id, value, group.mandals); }} prefix="Assign whole District" /></div><div className={styles.mandalAllocation}>{group.mandals.map(function (mandal) { return <label key={mandal.id}><span><strong>{mandal.name}</strong><small>{mandal.code}</small></span><CampaignerSelect disabled={Boolean(districtValue)} value={assignments[`M:${mandal.id}`] || ""} campaigners={campaigners} onChange={function (value) { setAssignments({ ...assignments, [`M:${mandal.id}`]: value }); }} /></label>; })}</div></article>; })}</div></div>}
      {!mandals.length && <div className={styles.empty}>No Mandals could be resolved. Complete this target’s Administrative crosswalk before creating a campaign.</div>}
